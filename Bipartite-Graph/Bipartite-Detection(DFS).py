...
THOUGHT PROCESS:
    ...
    1. Recall, DFS traverse the graph depth wise -> until finds a DEAD END
    2. Here, instead of visited array initialize a color array with -1 (-1=not colored yet)
    3. Start from source, color it
    4. Keep traversing the adjacent nodes and color it if yet not colored by making recursive call
...
NOTES:
    ...
    7. The algorithm could've been written using Iterative-DFS as well
...
def detect(graph, node, nodeCol, color):
    color[node] = nodeCol

    for adj_ver in graph[node]:
        if (color[adj_ver] == -1):
            if (detect(graph, adj_ver, not(color[node]), color) == False):
                return False
...
def isBipartite(graph, n):
    ...
    for node in range(1, n+1):
        if (color[node] == -1):
            if (detect(graph, node, 0, color) == False):
                return False
